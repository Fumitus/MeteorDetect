...
def auto_gamma(frame, target = 0.25):
    """Simple auto gamma: map median to target luminance (0..1)."""
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    med = np.median(gray) / 255.0 + 1e-6
    gamma = max(0.3, min(3.0, math.log(target, max(med, 1e-4))))
    lut = np.array([((i / 255.0) ** gamma) * 255 for i in range(256)]).astype("uint8")
    out = cv2.LUT(frame, lut)
    return out


def stabilize(prev_gray, gray):
    """Estimate translation+rotation via feature matching; return stabilized gray and warp matrix."""
    ...
    H, inliers = cv2.estimateAffinePartial2D(pts2, pts1, method=cv2.RANSAC, ransacReprojThreshold=3.0)
    if H is None:
        return gray, np.eye(3, dtype=np.float32)
    h, w = gray.shape
    stabilized = cv2.warpAffine(gray, H, (w, h),
                                flags=cv2.INTER_LINEAR,
                                borderMode=cv2.BORDER_REFLECT)
    H3 = np.eye(3, dtype=np.float32)
    H3[:2, :] = H
    return stabilized, H3

def hough_lines(mask, min_len=40, max_gap=4):
    # Probabilistic Hough; input must be an 8-bit edge image or binary
    lines = cv2.HoughLinesP(mask, rho=1,
                            theta=np.pi/180,
                            threshold=12,
...
class MeteorDetector:
    def __init__(self, args):
        self.args = args
        self.bg = None  # running average background (float32)
        self.prebuf = deque(maxlen=args.pre_frames)
        self.post_countdown = 0
        self.writers = []
        self.event_id = 0
        self.tracks = {}  # id -> {bbox, angle, frames}
...
def parse_args():
    p = argparse.ArgumentParser(description='Meteor detector for night videos')
    p.add_argument('--video', required=True, help='Path to input video (MP4/AVI)')
    p.add_argument('--out', default='detections', help='Output directory')
    p.add_argument('--auto-gamma', action='store_true', dest='auto_gamma', help='Enable auto gamma normalization')
    p.add_argument('--bg-alpha', type=float, default=0.01, help='Background update rate (0.005-0.05)')
    p.add_argument('--thresh', type=int, default=25, help='Brightness residual threshold (8-bit)')
    p.add_argument('--median', type=int, default=3, help='Median blur kernel (odd, 0 to disable)')
    p.add_argument('--open-kernel', type=int, default=1, help='Morphological open kernel size (0 to disable)')
    p.add_argument('--close-kernel', type=int, default=3, help='Morphological close kernel size (0 to disable)')
    p.add_argument('--min-length', type=int, default=40, help='Minimum Hough line length in pixels')
    p.add_argument('--max-gap', type=int, default=4, help='Max gap for HoughLinesP')
    p.add_argument('--line-thickness', type=int, default=2, help='Thickness when validating line region')
    p.add_argument('--min-area', type=int, default=20, help='Minimum contour area (px) near line')
    p.add_argument('--min-aspect', type=float, default=6.0, help='Min width/height aspect ratio (thin streak)')
    p.add_argument('--confirm-frames', type=int, default=2, help='Frames a candidate must persist to confirm')
    p.add_argument('--pre-frames', type=int, default=15, help='Frames to include before detection in clip')
    p.add_argument('--post-frames', type=int, default=30, help='Frames to include after last detection in clip')
    p.add_argument('--save-clips', action='store_true', help='Save MP4 clips for detections')
    p.add_argument('--downscale', type=float, default=1.0, help='Downscale factor (>1 speeds up, reduces res)')
    p.add_argument('--stabilize', action='store_true', help='Attempt basic inter-frame stabilization')
    p.add_argument('--show', action='store_true', help='Show debug windows')
    return p.parse_args()

def main():
    args = parse_args()
    det = MeteorDetector(args)
    det.run(args.video, args.out, save_clips=args.save_clips, stabilize_flag=args.stabilize)
